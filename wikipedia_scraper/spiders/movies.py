...
class MoviesSpider(scrapy.Spider):
    ...
    def parse(self, response):
        table_rows = response.css('.wikitable tr')[1:]
        for row in table_rows:
            ...
            loader.add_value('name', award_data[0])
            ...
            if not link:
                ...
                yield response.follow(link[0], self.parse_movie_links, cb_kwargs={ 'movie_item': movie_item })

    def parse_movie_links(self, response, movie_item):
        infoboxRows = response.css('table.infobox')
        if not infoboxRows:
            yield movie_item
        else:
            infoboxRows = infoboxRows[0].css('tr')
